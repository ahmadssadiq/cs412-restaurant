...
class VoterFilterForm(forms.Form):
    PARTY_CHOICES = [("all", "All")] + [
        (party, party) for party in PARTY_AFFILIATION_MAP.values()
    ]
    VOTER_SCORE_CHOICES = [("all", "All")] + [
        (score, score)
        for score in Voter.objects.values_list("voter_score", flat=True).distinct()
    ]
    YEAR_CHOICES = [(year, year) for year in range(1900, 2025)]

    party_affiliation = forms.MultipleChoiceField(
        choices=PARTY_CHOICES, widget=forms.CheckboxSelectMultiple, required=False
    )
    min_dob = forms.DateField(
        widget=forms.SelectDateWidget(years=range(1900, 2025)), required=False
    )
